...
const Navbar = () => {
  ...
  const handleNavClick = (item: string) => {
    ...
    const sectionMap: { [key: string]: string } = {
      "Home": "hero", "About Me": "developer-identity", "Journey": "professional-journey"
    };
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center px-3 sm:px-4 py-2 sm:py-2.5">
        <motion.button 
          ...
          <div className="flex flex-row gap-2">
            {menuItems.map((item) => (
              <motion.button
                ...
                className="px-4 py-2 text-sm font-medium text-foreground/80 hover:text-primary transition-all duration-300 uppercase"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                {t(item.key)}
