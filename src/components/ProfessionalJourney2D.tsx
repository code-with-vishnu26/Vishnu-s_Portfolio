...
const journeyData: JourneyNode[] = [
  { id: 1, role: "B.Tech Student", company: "Woxsen University", duration: "2022 - 2026", achievements: ["B.Tech in Computer Science Engineering", "77.2%\u00a0", "Active participant in hackathons and technical events", "Building strong foundation in programming and software development"], techStack: ["Python", "Java", "JavaScript", "Data Structures", "Algorithms"], color: "#3b82f6", icon: GraduationCap },
  ...
  { id: 3, role: "Full Stack Developer", company: "Personal Projects", duration: "2023", achievements: ["Built 5+ comprehensive full-stack applications", "Specialized in AI integration and cybersecurity", "Reduced security vulnerabilities by 60% through automated scanning", "Implemented blockchain-based password management system"], techStack: ["React.js", "Node.js", "MongoDB", "Docker", "Blockchain"], color: "#ec4899", icon: Code },
  { id: 4, role: "Aspiring Software Engineer", company: "Seeking Opportunities", duration: "2026– Future", achievements: ["Actively seeking internship and full-time opportunities", "Building professional network and industry connections", "Continuously learning emerging technologies", "Contributing to open-source projects"], techStack: ["Machine Learning", "Cloud Computing", "DevOps", "Microservices"], color: "#10b981", icon: Briefcase },
];
...
const ProfessionalJourney2D = () => {
  ...
  useEffect(() => {
    if (selectedJourney !== null) {
      ...
      // Hide common chatbot widgets while modal is open
      const chatSelectors = [
        "#lovable-chat-widget",
        "[data-chat-widget]",
        ".chat-widget",
  ...
  return (
    <section id="professional-journey" className="py-8 sm:py-10 md:py-14 relative z-10 overflow-hidden">
      <div className="container mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} className="text-center mb-6 sm:mb-8 md:mb-10">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 dark:from-blue-400 dark:via-purple-400 dark:to-pink-400 bg-clip-text text-transparent">My Professional Journey</h2>
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="relative z-10 grid sm:grid-cols-2 gap-3 sm:gap-4 md:gap-6 py-6 sm:py-8 md:py-12">
        ...
        {typeof document !== "undefined" && createPortal(
          <AnimatePresence>
            {selectedJourney && selectedData && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 dark:bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => setSelectedJourney(null)}>
                <motion.div initial={{ opacity: 0, scale: 0.8, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 50 }} transition={{ type: "spring", damping: 20, stiffness: 300 }} className="bg-card dark:bg-gradient-to-br dark:from-gray-900 dark:via-blue-900/30 dark:to-purple-900/30 rounded-2xl p-5 sm:p-6 md:p-8 max-w-lg sm:max-w-xl md:max-w-2xl w-full border border-border dark:border-white/10 shadow-2xl max-h-[85vh] overflow-y-auto relative" onClick={(e) => e.stopPropagation()}>
